 ...
 *  
 *  
 *  CR1 allows additional sticky interrupt sources in SR2 to be enabled. These sources fan into the RTC peripheral interrupt to optionally tell the CPU when (i) the RTC count has changed, (ii) a prescaled (modulo-1)  gated-clock event has occurred to advance the RTC count, (iii) a trim boundary (and thus trimming) has occurred, (iv) the 32-bit RTC count in {CNT1, CNT0} has rolled over and (v) the modulo-60 version of the RTC count has rolled over.
 *  
 *  
 *  
 *  
...
typedef struct _ADI_RTC_MOD_t {
  union {
    struct {
      ...
      unsigned int CNT0_4TOZERO :  5;  /**< Mirror of RTC Count 0 Register [4:0] */
    };
    uint16_t VALUE16;
  };
} ADI_RTC_MOD_t;
 ...
 *  
 *  
 *  Note that the number of valid bits (which can legitimately have values of one optionally written to them) of ALM2 equals the number of prescale bits specified by the CR1:PRESCALE2EXP field of the CR1 MMR. If a bit position in ALM2 is set to one, such that this is incompatible with the CR1:PRESCALE2EXP field, the whole value of ALM2 is treated by the RTC as if it is zero.
 *  
 *  
 ...
 *  
 *  
 *  Note that in RTC0, a value (typically zero) must still be written to ALM2 to invoke an alarm time definition via a triple write to {ALM1, ALM0, ALM2}. This applies even though RTC0 always treats the fractional part of an alarm time as zero.
 *  
 *  
 *  In contrast, on RTC1, full support for fractional alarm time is present, such that an alarm can be specified down to an individual 32 kHz clock cycle using ALM2.
 *  @{
 */
 ...
 *
 *  CR3OC is a control register for configuring enables related to 16-bit output-compare channel. CR3OC contains enables for both the output-compare function itself for each channel, as well as enables for whether an event on a channel should contribute to the interrupt lines from the RTC to both the CPU and the wake-up controller.
 *  
 *  Note that the 47-bit RTC_OC0 channel is not controlled by CR3OC. This is because RTCOC0 is a synonym for the main 47-bit RTC alarm (whose interrupt source is ALMINT), which is controlled by the CR0:ALMEN and CR0:ALMINTEN fields
 *  
 *  Note that this register only exists in RTC1. In RTC0, the register address is reserved and reads back as the register's reset value.
 *  @{
 */
...
typedef struct _ADI_RTC_OCMSK_t {
  union {
    struct {
      unsigned int RTCOCMSK   : 16;  /**< Concatenation of thermometer-encoded masks for the 16-bit output compare channels */
    };
    uint16_t VALUE16;
  };
} ADI_RTC_OCMSK_t;
 ...
 *
 *  OC1ARL contains the 16-bit reload value which is optionally (enabled by CR4OC:RTCOC1ARLEN) added to the cumulative value of OC1, visible in the OC1TGT register, whenever a enabled output compare event occurs on that channel. Note that only OC1 of all the output compare channels has this reload capability.
 *  
...
typedef struct _ADI_RTC_OC1_t {
  union {
    struct {
      unsigned int RTCOC1     : 16;  /**< RTC Output Compare 1 Channel. Scheduled alarm target time with optional auto-reload */
    };
    uint16_t VALUE16;
  };
} ADI_RTC_OC1_t;
...
typedef struct _ADI_RTC_SR5_t {
  union {
    struct {
      unsigned int WPNDSR0    :  1;  /**< Pending status of posted clearances of interrupt sources in RTC Status 3 Register */
 ...
 *  Note that OC1TGT only exists in RTC1. In RTC0, the register address is reserved and reads back as the register's reset value.
 *  @{
 */
...
typedef struct _ADI_RTC_OC1TGT_t {
  union {
    struct {
      unsigned int RTCOC1TGT  : 16;  /**< Current, cumulative target time for Output Compare Channel 1, taking account of any auto-reloading */
    };
    uint16_t VALUE16;
  };
} ADI_RTC_OC1TGT_t;
...
typedef struct _ADI_UART_COMIIR_t {
  union {
    struct {
      ...
      unsigned int STA        :  3;  /**< Interrupt status */
 ...
 *
 *  Be aware: internal UART baud generation counters are restarted whenever COMDIV register accessed by writing, regardless same or different value.
 *  @{
 */
...
typedef struct _ADI_ADC_CFG_t {
  union {
    struct {
      ...
      unsigned int VREFVBAT   :  1;  /**< To select Vbat (battery voltage) as Vref */
...
typedef struct _ADI_ADC_CNV_CFG_t {
  union {
    struct {
      ...
      unsigned int TMPSAMP    :  1;  /**< To enable temperature sensor sampling */
...
/** @defgroup TMP_SAMP Sampling Time for Temperature Sensor (TMP_SAMP) Register
 *  Sampling Time for Temperature Sensor (TMP_SAMP) Register.
 *
 *  
 *  @{
 */
...
/* ==========================================================================
 *! \struct ADI_ADC_TMP_SAMP_Struct
 *! \brief  Sampling Time for Temperature Sensor Register bit field structure
 * ========================================================================== */
#ifndef __ADI_NO_DECL_STRUCT_ADI_ADC_TMP_SAMP_t__
typedef struct _ADI_ADC_TMP_SAMP_t {
  union {
    struct {
      unsigned int TIME       :  8;  /**< Value of counter for sampling time 2 */
      unsigned int reserved8  :  8;
    };
    uint16_t VALUE16;
  };
} ADI_ADC_TMP_SAMP_t;
#endif /* !__ADI_NO_DECL_STRUCT_ADI_ADC_TMP_SAMP_t__ */
...
/** @defgroup TEMP1 Value of R1p25 (TEMP1) Register
 *  Value of R1p25 (TEMP1) Register.
 *
 *  
 *  @{
 */
...
/* ==========================================================================
 *! \struct ADI_ADC_TEMP1_Struct
 *! \brief  Value of R1p25 Register bit field structure
 * ========================================================================== */
#ifndef __ADI_NO_DECL_STRUCT_ADI_ADC_TEMP1_t__
typedef struct _ADI_ADC_TEMP1_t {
  union {
    struct {
      unsigned int R1p25      : 16;  /**< Value of R for calculation of temperature when Vref is 1.25 V */
    };
    uint16_t VALUE16;
  };
} ADI_ADC_TEMP1_t;
#endif /* !__ADI_NO_DECL_STRUCT_ADI_ADC_TEMP1_t__ */

/*@}*/

/** @defgroup TEMP2 Value of R2p5 (TEMP2) Register
 *  Value of R2p5 (TEMP2) Register.
 *
 *  
 *  @{
 */
...
/* ==========================================================================
 *! \struct ADI_ADC_TEMP2_Struct
 *! \brief  Value of R2p5 Register bit field structure
 * ========================================================================== */
#ifndef __ADI_NO_DECL_STRUCT_ADI_ADC_TEMP2_t__
typedef struct _ADI_ADC_TEMP2_t {
  union {
    struct {
      unsigned int R2p5       : 16;  /**< Value of R for calculation of temperature when Vref is 2.5 V */
    };
    uint16_t VALUE16;
  };
} ADI_ADC_TEMP2_t;
#endif /* !__ADI_NO_DECL_STRUCT_ADI_ADC_TEMP2_t__ */

/*@}*/

/** @defgroup TEMP_R Value of R_virtual (TEMP_R) Register
 *  Value of R_virtual (TEMP_R) Register.
 *
 *  
 *  @{
 */
...
/* ==========================================================================
 *! \struct ADI_ADC_TEMP_R_Struct
 *! \brief  Value of R_virtual Register bit field structure
 * ========================================================================== */
#ifndef __ADI_NO_DECL_STRUCT_ADI_ADC_TEMP_R_t__
typedef struct _ADI_ADC_TEMP_R_t {
  union {
    struct {
      unsigned int VIRTUAL    : 16;  /**< Value of R virtual reference for calculating temperature */
    };
    uint16_t VALUE16;
  };
} ADI_ADC_TEMP_R_t;
#endif /* !__ADI_NO_DECL_STRUCT_ADI_ADC_TEMP_R_t__ */
...
typedef struct _ADI_CRYPT_CFG_t {
  union {
    struct {
      ...
      unsigned int SHADATSRC  :  1;  /**< Select Data Input Source to SHA Engine */
      unsigned int RevID      :  4;  /**< Rev ID for Crypto on Glue Micro */
    };
    uint32_t VALUE32;
  };
} ADI_CRYPT_CFG_t;
 ...
 *
 *  This register is to be written before writing the last word to SHA input register. This is to inform the SHA engine that last word is about to be written by writing the SHA_LAST_WORD[0]. Also the no. of valid bits is to be programmed in SHA_LAST_WORD[5:1]
 *  @{
 */
...
typedef struct _ADI_CRYPT_CCM_NUM_VALID_BYTES_t {
  union {
    struct {
      ...
      unsigned int RESERVED_31_4   : 28;  /**< Reserved */
    };
    uint32_t VALUE32;
  };
